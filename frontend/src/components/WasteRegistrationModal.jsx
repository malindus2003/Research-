  ...
  return (
    <label htmlFor={id} className="block space-y-1.5">
      ...
      <select
        ...
        onChange={onChange}
        required={required}
        className="w-full rounded-xl border border-slate-700 bg-slate-950/80 px-3 py-2.5 text-sm text-slate-100 outline-none transition focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20"
      >
...
function Section({ title, children }) {
  return (
    <section className="space-y-4 rounded-2xl border border-slate-800 bg-slate-950/40 p-4 sm:p-5">
      <h4 className="border-b border-slate-800 pb-2 text-xs font-bold uppercase tracking-wider text-rose-400">
        {title}
      </h4>
      {children}
    </section>
...
export default function WasteRegistrationModal({ isOpen, onClose }) {
  ...
  return (
    ...
    >
      <div className="flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl shadow-black/60">
        <div className="flex items-start justify-between gap-4 border-b border-slate-800 px-5 py-4 sm:px-6">
          <div className="flex items-start gap-3">
            <span className="mt-0.5 rounded-xl bg-rose-500/10 p-2 text-rose-400">
              <ClipboardPlus className="h-5 w-5" />
            </span>
            <div>
              <h3 id="waste-registration-title" className="text-lg font-bold text-white">Register Wastage</h3>
              <p className="mt-0.5 text-xs text-slate-400">Record a manual waste entry for classification and measurement.</p>
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-2 text-slate-400 transition hover:bg-slate-800 hover:text-white focus:outline-none focus:ring-2 focus:ring-rose-500"
            ...
            <Section title="Measurement">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <label htmlFor="waste-weight" className="block space-y-1.5">
                  <span className="text-xs font-semibold text-slate-300">Weight<span className="ml-1 text-rose-400">*</span></span>
                  <input
                    id="waste-weight"
                    type="number"
                    min="0.01"
                    ...
                    placeholder="120"
                    className="w-full rounded-xl border border-slate-700 bg-slate-950/80 px-3 py-2.5 text-sm text-slate-100 outline-none transition placeholder:text-slate-600 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20"
                  />
                </label>
                <SelectField
                  id="weight-unit"
                ...
                <label htmlFor="waste-notes" className="block space-y-1.5 sm:col-span-2">
                  ...
                  <textarea
                    ...
                    className="w-full resize-y rounded-xl border border-slate-700 bg-slate-950/80 px-3 py-2.5 text-sm text-slate-100 outline-none transition placeholder:text-slate-600 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20"
                  />
                </label>
              </div>
            </Section>
        ...
        <div className="flex flex-col-reverse gap-3 border-t border-slate-800 bg-slate-900/95 px-5 py-4 sm:flex-row sm:justify-end sm:px-6">
          <button
            type="button"
            onClick={onClose}
            className="rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm font-semibold text-slate-200 transition hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-500"
          >
            Cancel
          </button>
          <button
